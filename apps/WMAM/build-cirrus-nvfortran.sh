...
function set_compile_options {
  ...
  if [[ "${BUILD}" == "release" ]]; then
    sed -i "s:FFLAGS =:FFLAGS = -I${MPI_HOME}/include -O3 -cpp -r8 -DCUDA_PINNED_MEMORY -DCUDA_STREAMS -tp=cascadelake -mp=gpu -cuda -gpu=cuda12.4,cc70:g" ${MAKEFILE}
  elif [[ "${BUILD}" == "debug" ]]; then
    sed -i "s:FFLAGS =:FFLAGS = -I${MPI_HOME}/include -g -O0 -cpp -r8 -DCUDA_PINNED_MEMORY -DCUDA_STREAMS -tp=cascadelake -mp=gpu -cuda -gpu=cuda12.4,cc70 -C -Mnobounds -ffpe-trap=invalid,zero,overflow,underflow,inexact -Ktrap=divz,denorm,inexact,inv,ovf,unf:g" ${MAKEFILE}
  fi
