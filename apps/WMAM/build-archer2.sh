...
function set_compile_options {
  ...
  if [[ "${BUILD}" == "release" ]]; then
    ...
    if [[ "${PRGENV}" == "cray" ]]; then
      sed -i "s:FFLAGS =:FFLAGS = -g -O3 -cpp -DCRAYPAT -h omp -h profile_generate:g" ${MAKEFILE}
    elif [[ "${PRGENV}" == "gnu" ]]; then
      sed -i "s:FFLAGS =:FFLAGS = -g -O3 -cpp -fopenmp -DCRAYPAT -fallow-argument-mismatch -std=legacy -fdefault-real-8 -fdefault-double-8:g" ${MAKEFILE}
    elif [[ "${PRGENV}" == "aocc" ]]; then
      sed -i "s:FFLAGS =:FFLAGS = -g -O3 -cpp -fopenmp -DCRAYPAT:g" ${MAKEFILE}
    fi
...
echo -e "\n\nBuilding ${WMAM_LABEL} ${WMAM_VERSION} (${BUILD}) using ${PRGENV} (CPE ${PE_RELEASE}) programming environment...\n\n"
