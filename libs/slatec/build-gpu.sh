...
BUILD=$1
GCC_VERSION=12.3.0
ERRMSG="Invalid syntax: build-gpu.sh release|debug"
...
module -s load gcc/${GCC_VERSION}-offload

PRFX=${HOME/home/work}
SLATEC_LABEL=slatec
SLATEC_VERSION=4.1
SLATEC_BUILD_ROOT=${PRFX}/projects/eCSE04-8/libs/${SLATEC_LABEL}/src
SLATEC_INSTALL_ROOT=${PRFX}/libs/${SLATEC_LABEL}/${SLATEC_VERSION}/GNU/${GCC_VERSION}-offload
SLATEC_INSTALL_PATH=${SLATEC_INSTALL_ROOT}/${PE_NAME}/${PE_RELEASE}/${BUILD}

echo -e "\n\nBuilding ${SLATEC_LABEL} ${SLATEC_VERSION} (${BUILD}) using ${PRGENV} (GCC ${GCC_VERSION}-offload) programming environment...\n\n"
