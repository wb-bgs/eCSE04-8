...
c	Subroutine Ysph_bi
c		Vincent Lesur 30/04/2002
c
c       For a all spherical harmonique between degre nlis,nlie
c       and a  colatitude, longitude, radius and time compute 
c       the value of the Y component basis  (Y Positive East) for internal
c       sources only. 
c       This basis is the derivative along longitude of an potential defined
c       on the sphere of radius "ra"
...
c   im.ne.0
        do im=1,nlie
          ...
          dc=-im*dsind(im*pos(2))
          ds=im*dcosd(im*pos(2))
