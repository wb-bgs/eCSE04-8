...
c       nm/lm           order and degree max 
c    outputs:
c       dlf             Legendre functions [dim min: max (2,lm-nm+1)]
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        subroutine  mklf_F(nm,lm,ds,dc,dlf)
c
        implicit none
c
        integer nm,lm,ierr,il
...
c
          ...
          dalpha=d1/dsqrt(d2)			!
c
          dlf(il+1)=dalpha*dlf(il)*dc		! 
     +                     -dbeta*dlf(il-1)	!leg. func. (nm+il-1,nm)
        enddo					
c
        return
        end
