...
c
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        subroutine opt_ghc_p2(path,itmax,npmax,npm,nd,npt,ppos,nn,bc
...
c
        ...
        character path*100
c
        integer i,ip,it,itm,iunit,ipth,itm_r,nb
...
c MP:  Check on direction
              ...
              write(*,*)'descent . gradient directions =',dd
              write(*,*)'Current STD before linear search = ',std
              write(iunit,*)'descent . gradient directions =',dd
...
c ALL: compute zz=2.A^t.W.A.ds
            ip=1
            call cptAtWAds_p(npmax,npm,npt,ip,nd,ppos,nb,FM,BS,bc,ds
...
c ALL: MP Broadcast the information to SPs if required
     ...
     >                                      ,MPI_COMM_WORLD,ierr)
          if(yon(3:4).eq.'ny')then
            call MPI_BCAST(gj,nb,MPI_DOUBLE_PRECISION,0
     >                                      ,MPI_COMM_WORLD,ierr)
            call MPI_BCAST(ghj,nb,MPI_DOUBLE_PRECISION,0
     >                                      ,MPI_COMM_WORLD,ierr)
          endif
        enddo
c
