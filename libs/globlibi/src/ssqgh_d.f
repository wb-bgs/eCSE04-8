...
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        subroutine ssqgh_d(npmax,npm,npt,ipg,nd,ppos,nb,FM,BS,bc
     >                        ,icov,jcov,cov,ddat,nt,xyzf,GJ,HJ)
c
        implicit none
c
        integer ip,npt,np,npmax,npm,nd,nb,icov(*),jcov(*)
        integer id,i,ipg,ipl,nt(*)
...
c Calculate the equations of condition
          call mkArows(np,npm,nt(ipl),nd,nb,ppos(1,ipl),bs,bc,aa)
c
...
c Update the G matrix and B vector
          call concoct_GJ(nt(ipl),FM,nb,np,vmf,dwgh,aa,ddif,GJ)
          call concoct_HJ(nt(ipl),FM,nb,np,vmf,dwgh,aa,HJ)
c
          ip=ip+np
        enddo
c
