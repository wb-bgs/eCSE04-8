...
c
      real*8 x(*),w(*),dpi,deps
     +       ,x1,x2,xm,xl,p1,p2,p3,pp,z,z1
c
      deps=1.d-15
      dpi=4.d0*datan(1.d0)
c
c     if(mod(n,2).ne.0)then
c       write(*,*)'Error  gauleg: n must be even'
c       stop 
c     endif
c
      m=int(float(n+1)/2.)
...
c
        ...
        it=0
        do while (dabs(z-z1).gt.deps.and.it.le.itmax)
          it=it+1
c
...
c Newton's iteration
        ...
        enddo
c
        if(it.ge.itmax)write(*,*)'Warning: convergence for zero:',i
c
