...
APP_EXE=${ROOT}/apps/${APP_NAME}/${APP_VERSION}/${PE_NAME}/${PE_VERSION}/debug/bin/${APP_EXE_NAME}
...
DDT_PARAMS="--offline --mem-debug=fast --check-bounds=before --mpi=slurm --np=${NCORES} --cwd=${APP_RUN_PATH}"
...
echo "DDT_PARAMS=${DDT_PARAMS}" >> ${APP_OUTPUT}

ddt ${DDT_PARAMS} ${APP_EXE} ${APP_PARAMS} &>> ${APP_OUTPUT}

RUN_STOP=$(date +%s.%N)
RUN_TIME=$(echo "${RUN_STOP} - ${RUN_START}" | bc)
echo -e "\nddt time: ${RUN_TIME}" >> ${APP_OUTPUT}
...
mv ${APP_RUN_PATH}/${APP_EXE_NAME}_*.html ${APP_RUN_PATH}/${SLURM_JOB_ID}/
