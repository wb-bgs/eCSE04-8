...
#NCU_PARAMS="--launch-skip 2 --launch-count 10 --kernel-name regex:\"cpt_dat_vals_p|ssqgh_dp\" --section MemoryWorkloadAnalysis --section SourceCounters --section SpeedOfLight --section SpeedOfLight_RooflineChart --export wmam-baseline-%q{SLURM_PROCID}"
...
#srun ${SRUN_PARAMS} ncu ${NCU_PARAMS} ${APP_EXE} ${APP_PARAMS}

srun ${SRUN_PARAMS} \
    ${APP_RUN_ROOT}/submit/${DEGREE}/cirrus/ncu_wmam.sh ${NTASKSPN} "${APP_EXE}" "${APP_PARAMS}"
...
mv ${APP_RUN_PATH}/wmam-baseline-*.ncu-rep ${APP_RUN_PATH}/${SLURM_JOB_ID}/
